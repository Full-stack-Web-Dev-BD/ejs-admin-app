...
router.get("/Adminsubject", middleware.isLoggedin, async function (req, res) {
  const form_status_list = await form_status.findOne({ name: "adminsubject" });
    ...
    .exec(function (err, returnedsubjectfields) {
      if (err) {
        ...
        subjectcat
          ...
          .exec(function (err, returnedcats) {
            if (err) {
              console.log(err);
            } else {
              res.render("adminsubject", {
                subjectadmin: returnedsubjectfields,
                subjectcats: returnedcats,
                formstatus: form_status_list,
                user: req.user,
                formlist,
              });
...
router.get(
  ...
  async function (req, res) {
    let finded = await form_status.findOne({ name: "adminsubject" });
    if (finded) {
      console.log(finded);
      return res.redirect("/adminsubject");
    }
    new form_status({
      name: "adminsubject",
    })
      .save()
      .then((doc) => {
        console.log("added in form status  ", doc);
...
router.post("/adminsubjectcat", middleware.isLoggedin, function (req, res) {
  ...
  var newadminsubjectcat = {
    subjectcatname: adminsubjectcat,
    subjectcatdetail: subjectcatdetail,
    subjectcatorder: subjectcatorder,
  };
      ...
      if (err) {
        ...
        subjectcat.find({}, function (err, returnedcats) {
          if (err) {
            ...
            res.render("admsubjectfieldedit", {
              editedsubjectfield: returnedsubjectfield,
              subjectcats: returnedcats,
            });
            let datetime = new Date();
            let logDate = datetime;
...
router.put("/adminsubject/:id", middleware.isLoggedin, function (req, res) {
  subjectfields.findByIdAndUpdate(
    req.params.id,
    {
      fieldorder: req.body.fieldorder,
      subjectcatname: req.body.catname,
      fieldname: req.body.fieldname,
      fieldinputtype: req.body.fieldinputtype,
      checkboxvalues: req.body.checkboxvalues.split(","),
      radiovalues: req.body.radiovalues.split(","),
    },
    function (err, returnedstudyfield) {
      if (err) {
        console.log(err);
        res.redirect("/adminsubject");
      } else {
...
router.put("/adminsubjectcat/:id", middleware.isLoggedin, function (req, res) {
  subjectcat.findByIdAndUpdate(
    req.params.id,
    {
      subjectcatname: req.body.catname,
      subjectcatdetail: req.body.subjectcatdetail,
      subjectcatorder: req.body.subjectcatorder,
    },
          ...
          subjectcat
            ...
            .exec(function (err, returnedsubjectcats) {
              if (err) {
                ...
                subject.findById(
                  req.params.id,
                  function (err, returnedsubject) {
                    if (err) {
                      console.log(err);
                    } else {
                      res.render("adminsubjecthomeedit", {
                        returnedsubjectfields: returnedsubjectfields,
                        returnedsubjectcats: returnedsubjectcats,
                        returnedsubject: JSON.parse(
                          JSON.stringify(returnedsubject)
                        ),
                      });
                      let datetime = new Date();
                      let logDate = datetime;
...
router.put(
  "/adminsubjecthomeedit/:id",
  middleware.isLoggedin,
  function (req, res) {
    subject.findByIdAndUpdate(
      req.params.id,
      req.body.returnedsubject,
      function (err, returnedsubject) {
        if (err) {
          console.log(err);
          res.redirect("/adminsubjecthome");
...
router.get("/Adminsubjecthome/:id/visit/new", async function (req, res) {
  ...
  subject.findById(req.params.id, function (err, subject) {
    if (err) {
      ...
      study
        .find({})
        .sort("Study_short_name")
        .exec(function (err, returnedstudy) {
          if (err) {
            console.log(err);
          } else {
            site
              .find({})
              .sort("Site_short_name")
              .exec(function (err, returnedsite) {
                if (err) {
                  console.log(err);
                } else {
                  phase
                    .find({})
                    .sort("Phase_number")
                    .exec(function (err, returnedphase) {
                      if (err) {
                        console.log(err);
                      } else {
                        visitfields
