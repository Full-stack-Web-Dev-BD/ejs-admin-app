...
router.get("/adminstudy", middleware.isLoggedin, async function (req, res) {
  const formlist = await form_status.find();
  const form_status_list = await form_status.findOne({ name: "adminstudy" });
  console.log(form_status_list);
  studyfields
    .find({})
    .sort("fieldorder")
...
router.get(
  ...
  async function (req, res) {
    let finded = await form_status.findOne({ name: "adminstudy" });
    if (finded) {
      console.log(finded);
      return res.redirect("/adminstudy");
    }
    new form_status({
      name: "adminstudy",
    })
...
router.put("/adminstudy/:id", middleware.isLoggedin, function (req, res) {
  studyfields.findByIdAndUpdate(
    req.params.id,
    {
      fieldorder: req.body.fieldorder,
      studycatname: req.body.catname,
...
router.get("/adminstudycat", middleware.isLoggedin, async function (req, res) {
  ...
  studycat
    .find({})
    .sort("studycatorder")
    .exec(function (err, returnedstudycat) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/adminstudycat", middleware.isLoggedin, function (req, res) {
  ...
  var newadminstudycat = {
    studycatname: adminstudycat,
    studycatorder: studycatorder,
    studycatdetail: studycatdetail,
  };
  //create a new studycategory and save it to the database.
...
router.put("/adminstudycat/:id", middleware.isLoggedin, function (req, res) {
  studycat.findByIdAndUpdate(
    req.params.id,
    {
      studycatname: req.body.catname,
      studycatdetail: req.body.studycatdetail,
      studycatorder: req.body.studycatorder,
    },
              ...
              if (err) {
                ...
                study.findById(req.params.id, function (err, returnedstudy) {
                  if (err) {
                    ...
                    res.render("adminstudyhomeedit", {
                      returnedstudyfields: returnedstudyfields,
                      returnedstudycats: returnedstudycats,
                      returnedstudy: JSON.parse(JSON.stringify(returnedstudy)),
                    });
                    let datetime = new Date();
                    let logDate = datetime;
                    let logUser = req.user.username;
...
router.put(
  "/adminstudyhomeedit/:id",
  middleware.isLoggedin,
  function (req, res) {
    study.findByIdAndUpdate(
      req.params.id,
      req.body.returnedstudy,
      function (err, returnedstudy) {
        if (err) {
          console.log(err);
          res.redirect("/adminstudyhome");
        } else {
