...
router.get("/adminsite", middleware.isLoggedin, async function (req, res) {
  const form_status_list = await form_status.findOne({ name: "adminsite" });
        ...
        sitecat
          ...
          .exec(function (err, returnedsitecats) {
            if (err) {
              ...
              let logAction =
                "Visited ADMINSITE - Checked configured fields for SITES";
              let newlog = {
                logDate: logDate,
                logUser: logUser,
                logAction: logAction,
...
router.get(
  ...
  async function (req, res) {
    let finded = await form_status.findOne({ name: "adminsite" });
    if (finded) {
      console.log(finded);
      return res.redirect("/adminsite");
    }
    new form_status({
      name: "adminsite",
    })
...
router.put("/adminsite/:id", middleware.isLoggedin, function (req, res) {
  sitefields.findByIdAndUpdate(
    req.params.id,
    {
      fieldorder: req.body.fieldorder,
      sitecatname: req.body.catname,
...
router.get("/adminsitecat", middleware.isLoggedin, function (req, res) {
  ...
  sitecat
    .find({})
    .sort("sitecatorder")
    .exec(function (err, returnedsitecat) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/adminsitecat", middleware.isLoggedin, function (req, res) {
  ...
  var newadminsitecat = {
    sitecatname: adminsitecat,
    sitecatorder: sitecatorder,
    sitecatdetail: sitecatdetail,
  };
...
router.put("/adminsitecat/:id", middleware.isLoggedin, function (req, res) {
  sitecat.findByIdAndUpdate(
    req.params.id,
    {
      sitecatname: req.body.catname,
      sitecatdetail: req.body.sitecatdetail,
      sitecatorder: req.body.sitecatorder,
    },
              ...
              if (err) {
                ...
                site.findById(req.params.id, function (err, returnedsite) {
                  if (err) {
                    ...
                    res.render("adminsitehomeedit", {
                      returnedsitefields: returnedsitefields,
                      returnedsitecats: returnedsitecats,
                      returnedsite: JSON.parse(JSON.stringify(returnedsite)),
                    });
                    let datetime = new Date();
                    let logDate = datetime;
                    let logUser = req.user.username;
...
router.put(
  "/adminsitehomeedit/:id",
  middleware.isLoggedin,
  function (req, res) {
    site.findByIdAndUpdate(
      req.params.id,
      req.body.returnedsite,
      function (err, returnedsite) {
        if (err) {
          console.log(err);
          res.redirect("/adminsitehome");
        } else {
