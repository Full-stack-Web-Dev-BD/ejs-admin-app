...
router.get("/adminvisit", middleware.isLoggedin, async function (req, res) {
  const form_status_list = await form_status.findOne({ name: "adminvisit" });
        ...
        visitcat
          ...
          .exec(function (err, returnedvisitcats) {
            if (err) {
              ...
              res.render("adminvisit", {
                visitadmin: returnedvisitfields,
...
router.get(
  ...
  async function (req, res) {
    let finded = await form_status.findOne({ name: "adminvisit" });
    if (finded) {
      console.log(finded);
      return res.redirect("/adminvisit");
    }
    new form_status({
      name: "adminvisit",
    })
...
router.get("/adminvisitcat", middleware.isLoggedin, async function (req, res) {
  ...
  visitcat
    .find({})
    .sort("visitcatorder")
    .exec(function (err, returnedvisitcat) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/adminvisitcat", middleware.isLoggedin, async function (req, res) {
  ...
  var newadminvisitcat = {
    visitcatname: adminvisitcat,
    visitcatorder: visitcatorder,
    visitcatdetail: visitcatdetail,
  };
...
router.post("/visit", middleware.isLoggedin, async function (req, res) {
  ...
  subject
    ...
    .sort("Visit_date")
    .exec(function (err, returnedsubjects) {
      ...
      if (err) {
        ...
        if (!returnedsubjects || !returnedsubjects.length) {
          ...
          req.flash(
            "error",
            "There is no SUBJECT matching the search criteria"
          );
          res.redirect("/visit");
        } else {
          res.render("visit", {
            ...
            .exec(function (err, returnedvisitcats) {
              if (err) {
                ...
                visit
                  ...
                  .populate("Subject")
                  .sort("Visit_date")
                  .exec(function (err, returnedvisit) {
                    if (err) {
                      console.log(err);
                    } else {
                      phase
                                    ...
                                    .exec(function (err, returnedstudy) {
                                      if (err) {
                                        ...
                                        res.render("adminvisitedit", {
                                          ...
                                          returnedvisitfields:
                                            returnedvisitfields,
                                          returnedvisitcats: returnedvisitcats,
                                          returnedvisit: JSON.parse(
                                            JSON.stringify(returnedvisit)
                                          ),
                                        });
                                      }
                                    });
