...
//====== AUTH ROUTES START ========================
router.get("/adminuser", middleware.isLoggedinAdmin, async function (req, res) {
  ...
  async function (req, res) {
    ...
    user.findById(req.params.id, function (err, returneduser) {
      if (err) {
        ...
        res.render("admuseredit", { formlist, user: returneduser });
      }
    });
  }
);
          ...
          returneduser.setPassword(req.body.password, function (err) {
            ...
            returneduser.save(function (err) {
              ...
              if (err) {
                ...
                req.flash(
                  "success",
                  "Password changed for: " + returneduser.username
                );
                res.redirect("/adminuser");
              }
            });
...
router.put("/adminuser/:id", middleware.isLoggedinAdmin, function (req, res) {
  user.findByIdAndUpdate(
    req.params.id,
    { fullname: req.body.fullname, role: req.body.role },
...
router.get("/AdminLogs", middleware.isLoggedinAdmin, async function (req, res) {
  const formlist = await form_status.find();

  logs
    .find({})
    .sort({ logDate: "desc" })
    .exec(function (err, returnedlogs) {
      if (err) {
        console.log(err);
        res.redirect("/AdminLogs");
      } else {
        res.render("adminlogs", { formlist, logs: returnedlogs });
      }
    });
});
