...
router.get("/adminphase", middleware.isLoggedin, async function (req, res) {
  const form_status_list = await form_status.findOne({ name: "adminphase" });
...
router.get(
  ...
  async function (req, res) {
    let finded = await form_status.findOne({ name: "adminphase" });
    if (finded) {
      console.log(finded);
      return res.redirect("/adminphase");
    }
    new form_status({
      name: "adminphase",
    })
...
router.put("/adminphase/:id", middleware.isLoggedin, function (req, res) {
  phasefields.findByIdAndUpdate(
    req.params.id,
    {
      fieldorder: req.body.fieldorder,
      phasecatname: req.body.catname,
    ...
    function (err, returnedphasefield) {
      if (err) {
        console.log(err);
        res.redirect("/adminphase");
      } else {
...
router.get("/adminphasecat", middleware.isLoggedin, function (req, res) {
  ...
  phasecat
    .find({})
    .sort("phasecatorder")
    .exec(function (err, returnedphasecat) {
      if (err) {
        console.log(err);
      } else {
...
router.post("/adminphasecat", middleware.isLoggedin, function (req, res) {
  ...
  var newadminphasecat = {
    phasecatname: adminphasecat,
    phasecatorder: phasecatorder,
    phasecatdetail: phasecatdetail,
  };
...
router.put("/adminphasecat/:id", middleware.isLoggedin, function (req, res) {
  phasecat.findByIdAndUpdate(
    req.params.id,
    {
      phasecatname: req.body.catname,
      phasecatdetail: req.body.phasecatdetail,
      phasecatorder: req.body.phasecatorder,
    },
...
router.put("/phaseedit/:id", middleware.isLoggedin, function (req, res) {
  phase.findByIdAndUpdate(
    req.params.id,
    req.body.returnedphase,
    function (err, returnedphase) {
      if (err) {
        console.log(err);
        res.redirect("/phase");
      } else {
